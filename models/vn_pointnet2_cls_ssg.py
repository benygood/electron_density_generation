...
class get_model(nn.Module):
    def __init__(self, last_npoint=10, atom_num_per_last_point = 10, atom_type_num = 10, normal_channel=True):
        ...
        self.sa2 = PointNetSetAbstraction(npoint=128, radius=0.3, nsample=20, in_channel=128//3 * 2 + 1, mlp=[128//3, 128//3, 256//3], group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=last_npoint, radius=0.5, nsample=8, in_channel=256//3 * 2 + 1, mlp=[256//3, 256//3, 512//3], group_all=False)
        self.conv_coord = nn.Conv1d(512//3*2*3, atom_num_per_last_point*3*16, 1)
        self.conv_coord_bn = nn.BatchNorm1d(atom_num_per_last_point*3*16)
        self.conv_type = nn.Conv1d(512//3*2*3, atom_num_per_last_point*16, 1)
