...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False):
    ...
    Input:
        ...
        points: input points data, [B, N, D, 3]
    Return:
    ...
    grouped_xyz_norm = grouped_xyz_norm.unsqueeze(-2)
...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all):
        ...
        self.std_feature = VNStdFeature(mlp[-1]*2, dim=4, normalize_frame=False)
    ...
    def forward(self, xyz, points):
        ...
        Input:
            xyz: input points position data, [B, N, 3]
            points: input points data, [B, N, D, 3], when init, D=1
        Return:
        ...
        # new_points: sampled points data, [B, npoint, nsample, 1+D, 3]
        new_points = new_points.permute(0, 3, 4, 1, 2) # [B, 1+D, 3, npoint, nsample]
        for i, conv in enumerate(self.mlp_convs):
            new_points = conv(new_points)

        new_points = torch.max(new_points, -1)[0]
        new_points_mean = new_points.mean(dim=-1, keepdim=True).expand(new_points.size())
        x = torch.cat((new_points, new_points_mean), 1)
